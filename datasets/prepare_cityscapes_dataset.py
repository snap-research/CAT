...
def check_matching_pair(segmap_path, photo_path):
    segmap_identifier = os.path.basename(segmap_path).replace(
        '_gtFine_color', '')
...
def process_cityscapes(gtFine_dir,
                       leftImg8bit_dir,
                       output_dir,
                       phase,
                       table_path=None):
    ...
    for i, (segmap_path,
            photo_path) in enumerate(tqdm(zip(segmap_paths, photo_paths))):
        ...
        if f is not None:
            rel_segmap_path = os.path.relpath(
                segmap_path, os.path.dirname(os.path.abspath(gtFine_dir)))
            rel_photo_path = os.path.relpath(
                photo_path, os.path.dirname(os.path.abspath(leftImg8bit_dir)))
            f.write('%d %s %s\n' %
                    (i, rel_segmap_path.replace('_color',
                                                '_trainIds'), rel_photo_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gtFine_dir',
                        type=str,
                        required=True,
    ...
    parser.add_argument(
        ...
        help=
        'Generate a mapping table to map the generated images to the original images. '
    ...
    process_cityscapes(opt.gtFine_dir,
                       opt.leftImg8bit_dir,
                       opt.output_dir,
                       "val",
