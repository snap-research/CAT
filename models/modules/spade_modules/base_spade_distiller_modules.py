...
class BaseSPADEDistillerModules(SPADEModelModules):
    ...
    def create_optimizers(self):
        ...
        optimizer_D = torch.optim.Adam(list(self.netD.parameters()),
                                       lr=D_lr,
                                       betas=(beta1, beta2))
    ...
    def compute_G_loss(self, input_semantics, real_B):
        with torch.no_grad():
            Tfake_B, Tacts = self.netG_teacher(
                input_semantics, mapping_layers=self.mapping_layers)
        Sfake_B, Sacts = self.netG_student(input_semantics,
                                           mapping_layers=self.mapping_layers)
        loss_G_distill, losses = self.calc_distill_loss(Tacts, Sacts)
        pred_fake, pred_real = self.discriminate(input_semantics, Sfake_B,
                                                 real_B)
    ...
    def compute_D_loss(self, input_semantics, real_B):
        ...
        pred_fake, pred_real = self.discriminate(input_semantics, fake_B,
                                                 real_B)
    ...
    def load_networks(self, verbose=True, teacher_only=False):
