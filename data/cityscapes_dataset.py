...
class CityscapesDataset(BaseDataset):
    ...
    def get_paths(self, opt):
        ...
        label_paths = [
            p for p in label_paths_all if p.endswith('_labelIds.png')
        ]
        ...
        if not opt.no_instance:
            instance_paths = [
                p for p in label_paths_all if p.endswith('_instanceIds.png')
            ]
        else:
            instance_paths = []
    ...
    def __getitem__(self, index):
        ...
        label_tensor[label_tensor ==
                     255] = self.opt.input_nc  # 'unknown' is opt.input_nc
        ...
        if self.opt.no_instance:
            ...
            if not self.opt.load_in_memory or self.instance_cache.get(
                    index) is None:
